 ...
 *
 * Usage: node setup.mjs [--check] [--yes]
...
export function runSetup(argv) {
    ...
    if (checkPnpm()) {
      ...
      fail(
        "pnpm — the install did not leave a usable pnpm >= " +
          `${pnpmPin} on PATH (open a new shell and re-run node setup.mjs)`,
      );
    }
  } else {
    fail(
  ...
  if (commandExists("uv")) {
    ...
    if (commandExists("uv")) {
      ...
      fail(
        "uv — the install did not leave uv on PATH (open a new shell and re-run node setup.mjs)",
      );
    }
  } else {
  ...
  if (checkOnly) {
    if (existsSync(join(REPO_ROOT, "node_modules"))) {
      ...
      fail("dependencies — run node setup.mjs (or pnpm install)");
    }
    ...
    if (hooksInstalled) {
      ...
      fail(
        "git hooks — run node setup.mjs (or pnpm install; its prepare script runs lefthook install)",
      );
    }
    if (chromiumProvisioned()) {
      ok("Playwright Chromium provisioned");
    } else {
      fail("Playwright Chromium — run node setup.mjs (or pnpm exec playwright install chromium)");
    }
  } else if (pnpmOk) {
  ...
  console.log(
    `  ${C_ERR}${failedCount()} problem(s)${C_OFF} — fix the lines marked missing above, then re-run node setup.mjs`,
  );
  return 1;
}
