...
- **The body may carry agent attribution; it may never carry a session link.** A "Generated with Claude Code" byline or a `Co-Authored-By: Claude` trailer is welcome — a reader of a public thread is entitled to know a change was agent-written. What stays out is `Claude-Session:` or a `https://claude.ai/code/session_…` URL: that points into a private transcript, and a PR body is public. `.claude/settings.json` → `attribution.sessionUrl` stops the harness appending one, and `.claude/hooks/block-session-links.mjs` denies a publishing call — `gh` or GitHub MCP, inline body or `--body-file` — whose text still contains one. A denial there means the link needs removing, not a workaround.
