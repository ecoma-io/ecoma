...
Five neighbours own different objects, and this file owns none of theirs:
...
| `/hunt-issues`                                                                             | real defects in one project's running code      | a defect is the built thing being wrong; this file judges the workspace's declarations being false — a repo can conform and still ship bugs |
...
| **"Audit the repo" is wider than this object** — per-project defect quality (a wrong branch in a primitive, a leaky adapter) appears in no group here, and a run of this rubric alone reads as a full audit | The neighbour table routes it: a whole-repo sweep is this rubric over the workspace **plus** `/doctrine-review` over the ceiling **plus** `/hunt-issues` rotated over the project trees — three instruments, and a report that ran fewer says which it skipped |
