...
`test run scope × time-travel query (bảng production đọc as-of log-position có thấy write của test không?)` · `tag release khi trunk private và cây public lệch nhịp` · `UI attended offline × Action Item đã claim (lease hết TTL khi máy mất mạng)` · `migrations[].down của block × instance đang chạy pin digest cũ` · `environment: test × pool filler của một Role (ai chặn gán sai — engine hay policy?)`.
