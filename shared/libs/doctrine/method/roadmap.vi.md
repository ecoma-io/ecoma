...
| **G3** | Trigger/Channel + Party/external-filler **chạy được** (gate chạy-được duy nhất, không phải freeze văn bản), và **đóng băng theo hai pha đã khai**: Trigger đủ mọi type đã bật cộng `response_mode: sync` khi M0 có nó chạy, rồi Channel, Party và external filler khi M2 mở | Track C (dogfood funnel)                                                                                 | Thấp — 1 track                                             |
...
**Chốt**: M1 làm vế trên, **không** làm vế dưới. Đây **không phải "thu hẹp giá trị"** như tưởng — wedge là RPA standalone cho người dùng đơn, mà người dùng đơn **đang ngồi ngay đó**; hàng đợi duyệt là khái niệm của tổ chức nhiều người, thuộc Track E. Nói cách khác: cắt đúng khớp thì M1 **không mất gì**. Hệ quả: **ADR-0005 phải sửa cụm từ**, và RPA NS §5 biên cứng #2 vẫn nguyên (mọi hành động **lao động** đi thẳng engine API) — xác nhận-trong-phiên không phải hành động lao động, nó là điều khiển phiên.

6. Milestone ICP-gated (M5/M6) **không có track riêng** — cổng của chúng là bằng chứng thị trường (§3b), không phải giao diện kỹ thuật.
7. **Gate = freeze văn bản + conformance test suite chạy độc lập.** Án văn: hai team đọc cùng một interface đóng băng vẫn implement khác nhau — đúng _định nghĩa_ major của R5 ("hai kỹ sư đọc sẽ implement khác nhau"); văn bản không đủ, chỉ test suite mới là trọng tài máy kiểm được. Hệ quả: track qua gate = pass suite, không phải "đọc kỹ rồi"; suite chính là chỗ **Process test harness** trả giá trị đầu tiên (§5: nhảy lên #2). Suite của gate cũng version hóa — đổi suite = đổi giao diện = breaking.

   **Một freeze khai phạm vi của chính nó, và phạm vi suite của nó, trong cùng một hành vi.** Một tài liệu mang freeze cho đúng phần của nó có đặt tên giao diện; phần còn lại của tài liệu đó vẫn mở, và phần nào bị đóng băng thì được viết trong chính trường frontmatter `frozen-scope` của tài liệu. Không gì ngoài review giữ được sự thu hẹp đó — executor đọc `status` và `gate`, và **không dòng code nào đọc `frozen-scope`, kể cả để in ra** — nên một `frozen-scope` trôi khỏi văn bản là một finding của review, không phải build đỏ. **Một gate mà chủ thể của nó tới qua hai milestone thì khai cả hai pha ngay tại lần freeze đầu**: một sự nới ra đã khai tại lúc freeze là giữ lời hứa, một sự nới ra không khai là cái break mà luật này cấm.

8. **Giới hạn của song song là số interface chịu được freeze sớm**, không phải số người. Freeze non → chi phí đổi nhân theo số track tiêu dùng (cột cuối bảng gate). Nghi ngờ giao diện chưa chín → **không mở track**, chấp nhận tuần tự — tuần tự rẻ hơn breaking lan.
...
- **Exit-litmus**: RPA NS §9 (10 câu) + L5 5 spec RPA (15 câu) — đặc biệt #6 _cùng binary cùng đường effect_, #5 _secret không bao giờ vào log/screenshot/context_, #9 _không kênh điều khiển thường trực_.
- **Spec treo CHẶN M1** → ✅ **HẾT CHẶN **: Vault tầng 1 · **Release & Compatibility** · **charter `deploy/`**. M1 exit-litmus nay **+9 câu** (L5 của Release & Compat) và **+9 câu** charter deploy — _charter litmus đo tại M1, cùng lớp với Website Charter §6 ở M2_.
...
- **Spec treo CHẶN M6**: **[quota-scheduling](../spec/quota-scheduling.md)** — đã
  viết; M6 không được nhận việc dưới trần tenant trước khi nó có hiệu lực ·
  **charter `cloud/`**, spec duy nhất M6 còn phải chờ.
...
**Cách đọc cột gate**: nó gọi tên cổng ◆ mà hàng đó _gắn vào_, và cột Nhà nói chiều gắn chạy theo hướng nào. Một hàng **cấu thành** một freeze thì nói thẳng trong ô Nhà của nó ("đây chính xác là cái ◆G0 đóng băng"); mọi hàng khác là **được cởi trói bởi** cổng mà nó gọi tên. Vậy nên một hàng không phải được giao _tại_ cổng của nó — A.5 được cởi trói bởi ◆G0, dựng bên trong M0, và việc nó chạy được mới là cái ◆G3 đóng trên.

| ID   | Cụm end-state (nguồn canonical)                                                          | Track | Gate | Nhà trong roadmap                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---- | ---------------------------------------------------------------------------------------- | ----- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| A.5  | Trigger đủ type đã bật **+ sync-response BaaS**                                          | A     | ◆G0  | M0 — được cởi trói bởi ◆G0; việc nó chạy được là cái ◆G3 đóng trên                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| A.20 | **Workspace — chiều đa-khách-hàng**: nhiều workspace + chiều workspace trong calibration | A     | ◆G0  | M5 — phân vùng mềm của Tenant §3, mang chiều workspace của nhóm C6. Vô hình ở n=1, và **không** phải ranh giới bảo mật — nên bỏ nó là mất một chiều tách biệt, không phải một sự đơn giản hoá                                                                                                                                                                                                                                                                                                     |
...
| D.3  | Gói block dọc đầu tiên — nội dung first-party do Hub mang                                | D     | ◆G2  | M5 — ICP-gated (§3b) theo ngành dọc đã xác nhận; registry và index của D.1 không phụ thuộc vào nó                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| A.18 | **Release & Compatibility** (train, negotiation, upgrade/rollback, EOL, suite version)   | A     | —    | **M1** — exit-litmus +9 câu L5; Q3 vào `nx.json` trước app đầu tiên                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| R.2  | Publishing policy + luật sư (SUL/CLA/EULA/trademark)                                     | R     | —    | §5 — song song, chặn contributor đầu tiên & mọi publish. Văn bản đã có và **`dev-cli check-contributor-record`** thi hành luật ký nhận; Corporate CLA đang có hiệu lực, còn Enterprise License đã nghỉ và một biên repository mang tầng đó thay cho nó (sổ license của index là hồ sơ) — phần còn lại là điều khoản `cloud/` độc quyền và các văn bản EULA, trademark. Vòng luật sư cho cả bộ là **hoãn chứ không phải đang chờ** — không có ngân sách, rủi ro ôm một cách có ý thức (index → L9) |
| R.3  | **Di cư tài liệu vào repo** (`doctrine/` + bề mặt đọc)                                   | R     | —    | Xong — cây đã ở trong repo; lịch sử git là bản ghi                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| R.4  | **Board ↔ roadmap** (luật hai chiều §0)                                                  | R     | —    | Nửa file: **`dev-cli check-roadmap-ids`** — ID không trùng, track/gate/milestone đều resolve về §1b và §4. **Tái sử dụng số thì gate không thấy** (một ảnh chụp không phân biệt được) và ở lại với người review. Nửa board: **`repo-care audit-roadmap-labels`** — mọi nhãn `roadmap:`/`track:`/`gate:`/`milestone:` của từng card phải resolve về tài liệu này; chỉ đọc, vì gắn lại nhãn sau một lần đổi tên là phán đoán về chỗ công việc đã dời tới                                            |
| R.5  | Build/branch/CI (ô `build` P1) + executor conformance suite                              | R     | —    | **playbook giao hàng (không công bố)**; executor là **`dev-cli conformance`** — đọc freeze và suite của từng gate từ cây, và fail một freeze không có suite (luật #7)                                                                                                                                                                                                                                                                                                                             |
