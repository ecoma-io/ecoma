...
| Tầng 3 — Human surface = **Work Surface** (My-Work/Org-Work, diff, mobile — spec ✅)    | Track E sau ◆G4 (= freeze projection read-API của spec); tối thiểu là điều kiện exit M0; đầy đủ dần tới M5                                                                                                            |
| **Tầng 4 — Pair-design**                                                                | **M3** _(trước đó mồ côi; chặn M4)_                                                                                                                                                                                   |
...
| Trigger đủ type đã bật **+ sync-response BaaS**                                         | **M0**; `message_in` + Channel + external-filler → M2                                                                                                                                                                 |
| Subsystem tầng 1: Event Log · Artifact Store · Lease · **Vault/key-store** (spec ✅)    | M0                                                                                                                                                                                                                    |
| Tenant & Identity core + vòng đời §2b                                                   | M0 (purge đầy đủ tựa lên key-store M0)                                                                                                                                                                                |
| Working Data: DataTable / Labor Analytics + BYO-export                                  | M2 / **M5 đủ**                                                                                                                                                                                                        |
...
| Metering (cơ chế) / Pricing (policy)                                                    | M0 exit tường minh / M6                                                                                                                                                                                               |
...
| **Calibration data model** (CalKey/Cell/estimator identity — spec ✅)                   | **M0** — điều kiện exit (litmus #3 "một thang tin cậy"); M0 exit +5 câu L5.                                                                                                                                           |
| **Test harness — hai vai tách bạch** (spec ✅)                                          | **vai hạ tầng (test mode + conformance suite) → M0**, là điều kiện của mọi gate ◆G · **vai sản phẩm (bề mặt "chạy thử" của user) → Track E sau ◆G4, đủ ở M3**. Litmus harness **đo tại gate, không cộng vào M0 exit** |
...
| **Deploy & Operations** (ô `deploy`/`operate`/`sunset` của P1; backup/restore; khóa-DR) | **[deploy](../charter/deploy.md)** — gắn **M1**; lệnh `check-backup-key-isolation` gắn PR tạo `deploy/`                                                                                                               |
| **Di cư tài liệu vào repo** (`doctrine/` + `website/apps/docs`)                         | **kế hoạch di cư (không công bố)** — 7 giai đoạn, 8 PR; **Track R**                                                                                                                                                   |
