...
| **G3** | Trigger, Channel, Party and external filler **actually running** — the only runnable gate rather than a text freeze, and **frozen in two declared phases**: Trigger with every enabled type plus `response_mode: sync` when M0 has it running, then Channel, Party and the external filler when M2 opens | Track C (the dogfood funnel)                                                                                                                            | Low — one track                                                          |
...
7. **A gate is a text freeze plus a conformance test suite that runs
   ...
   **A freeze declares its own scope, and its suite's scope, in the same act.**
   A document carries the freeze for the part of itself that names the
   interface; the rest of that document stays open, and which part is frozen is
   written in the document's own `frozen-scope` frontmatter field. Nothing but
   review holds that narrowing — the executor reads `status` and `gate`, and no
   code reads `frozen-scope` at all, not even to print it — so a `frozen-scope`
   that drifts from the text is a review finding, not a red build. **A gate
   whose subject arrives across two milestones declares both phases at the first
   freeze**: a growth declared at the freeze is the promise being kept, an
   undeclared one is the break this law forbids.
...
- **Blocking specifications**: **[quota-scheduling](../spec/quota-scheduling.md)**
  — written; M6 may not admit work against a tenant ceiling before it is in
  force · the **`cloud` charter**, the one specification M6 still waits on.
...
**Reading the gate column**: it names the ◆ gate a row is _coupled to_, and the
Home column says which way the coupling runs. A row that **constitutes** a
freeze says so in its Home cell ("this is precisely what ◆G0 freezes"); every
other row is **unblocked by** the gate it names. A row is therefore not
delivered _at_ its gate — A.5 is unblocked by ◆G0, built inside M0, and its
running is what ◆G3 closes on.

| ID   | End-state cluster (canonical source)                                                                             | Track | Gate | Home in the roadmap                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---- | ---------------------------------------------------------------------------------------------------------------- | ----- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| A.5  | Trigger, every enabled type **plus sync-response BaaS**                                                          | A     | ◆G0  | M0 — unblocked by ◆G0; its running is what ◆G3 closes on                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| A.20 | **Workspace as the multi-client dimension** — several workspaces plus the workspace dimension in calibration     | A     | ◆G0  | M5 — the soft partition of Tenant §3, carrying group C6's workspace dimension. Invisible at n=1, and **not** a security boundary, so removing it is a lost separation dimension rather than a simplification                                                                                                                                                                                                                                                                                                                                       |
...
| D.3  | The first vertical block bundle — first-party content carried by the Hub                                         | D     | ◆G2  | M5 — ICP-gated (§3b) on the confirmed vertical; the registry and index of D.1 do not depend on it                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| R.2  | Publishing policy plus legal review (SUL, CLA, EULA, trademark)                                                  | R     | —    | §5 — in parallel; it blocks the first contributor and every publication. The texts exist and **`dev-cli check-contributor-record`** enforces the acceptance rule; the Corporate CLA is in force, and the Enterprise License is retired with a repository boundary carrying its tier instead (the index's licence ledger is the record) — the residue is the proprietary `cloud/` terms and the EULA and trademark texts. Counsel review of the set is **deferred rather than pending** — no budget for it, the risk carried knowingly (index → L9) |
