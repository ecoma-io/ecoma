...
**Lattice và grant gác hai cửa khác nhau, không cửa nào thay được cửa kia.** `view_calibration` quyết một principal **bên trong** tenant có được đọc dữ liệu đánh giá hay không; classification quyết cái gì được **rời đi**. Người giữ grant chạm tới mức phân loại của cell nhưng **không** giữ `view_calibration` bị **grant** từ chối chứ không phải bị floor từ chối — đọc con số dẫn xuất vẫn là đọc calibration. Lattice thêm đúng nửa mà grant chưa từng có: một cú kiểm egress cho con số sau khi nó được vẽ ra, và một read event ghi lại cú thử theo cả hai chiều.

Lý do phải nói ra — và nói như **lịch sử**, vì đoạn này mô tả cái bỏ sót ĐÃ là gì, không phải cái luật nay cho phép. Calibration là **nguồn duy nhất có phân loại về bản chất mà vắng khỏi lattice**, và lỗ đó không nhìn thấy được từ cả hai phía: `view_calibration` gác **cell**, còn một con số _dẫn xuất_ từ cell không phải cell — nên **trước luật này** nó không kế thừa floor và không để lại read event, và confidence render cạnh một con số hướng client có thể băng sang người xem không giữ Role nào trong tenant, **do bỏ sót chứ không do ai quyết định**. **Dưới luật này**, con số đó mang floor của các cell đứng sau nó và việc đọc nó phát read event. Gia nhập lattice vẫn **không** quyết định con số đó có được hiện hay không: mặc định từ chối, và nới nó vẫn đúng là quyết định `calibration_visibility_policy` như cũ.
...
| Con số dẫn xuất tới tay người xem không có grant | **Hai cú kiểm, không phải một**: grant `view_calibration` cho người đọc bên trong tenant, và floor propagation vào leakage gate cho con số rời tenant — mỗi cú bắt đúng thứ cú kia không bắt được; read event theo mức ghi lại cú thử ở cả hai chiều (§6) | Cú đọc hoặc cú egress bị từ chối và được ghi; muốn con số đó ra ngoài thật thì đường là `calibration_visibility_policy`, không bao giờ là một bản sao không phân loại |
