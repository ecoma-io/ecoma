...
- **"Cùng một actor" là thứ được khai, vì so filler identity không thôi trên thực tế là bất đối xứng.** Guard mang `comparison_basis` — gọi đủ tên chứ không gọi `basis`, vì `basis` đã mang nghĩa loại của một Judgment (§3) và một chiều của cell calibration, và nghĩa thứ ba trên cùng một chữ là bẫy cho người đọc. Nó nhận `filler` — so danh tính filler, mặc định và đúng hành vi hiện nay — hoặc `principal` — so **principal chịu trách nhiệm**. Thiếu lựa chọn này, người đặt một agent vào giữa mình và công việc nhận được một bảo đảm mà người làm trực tiếp không có: một nhánh `if human` xuất hiện dưới dạng lỗ chứ không dưới dạng luật, trong khi litmus #3 của Role đòi "mọi đường, kể cả qua hai role". Chọn basis nào vẫn là giá trị template, vì cần tách bao nhiêu là policy.
- **Principal chịu trách nhiệm được ghi ở đâu — vì một phép so không đọc được một cạnh chưa ai viết.** Nạp một Filler vào pool của một Role là một thay đổi membership, và thay đổi membership là **event mang danh tính principal đầy đủ** (Tenant & Identity §4) — nên principal chịu trách nhiệm của một filler là **principal của chính event nạp đó**, giải tại lúc nạp và giải lại khi pool đổi. Của người là chính họ. Của agent hay rule là người đã nạp nó; nơi một filler `code` có khai tác giả thì đó là lời khai trong manifest, còn principal chịu trách nhiệm vẫn là người nạp — vì hành vi của chính tenant mới là thứ tenant chịu được trách nhiệm. Đây là **cạnh duy nhất** luật này thêm vào, và nó được nói ra chứ không được giả định.
