...
| PII                | Rows are events, so **crypto-shredding is inherited unchanged** from Event Log §4. **A table definition declares which columns are subject-keyed** (encrypted under a data-subject key); business columns live under the tenant DEK and survive a shred, personal columns die with the subject key (the split of Tenant & Identity §6, made mechanical for a table). A shred does not drop the row or repunch the log: the subject-keyed columns **project as an explicit `shredded` value** in every projection _and_ in a time-travel read as of any position — a reader at a pre-shred position cannot see what the key no longer decrypts, so rebuild-equivalence (Event Log §3) holds **for the currently-readable key set**, and the margin-per-client figures recompute with the delta explained by the shred entry rather than silently changing. **Subject-keyed-ness is not a classification**, and the export gate reads only classification: a subject-keyed column carries **the floor of the subject key** and, undeclared, does not egress (§4) |
...
**A subject-keyed column does not leave in the clear by default.** Egress reads
`classification` and nothing else, while subject-keyed-ness is a **separate axis**
declared by the table definition (§1) — untied, a personal column classified
`internal` walks out through a destination cleared for `internal`, and the shred
that follows reaches nothing. A subject-keyed column therefore carries **the floor
of the subject key**; undeclared, it does not egress. Exporting one in the clear is
the ordinary declassification-through-a-Gate route (Knowledge §3): an actor, an
event, a reason. **The limit, stated because this document states its limits**:
from that moment the plaintext copy is outside the key lifecycle Event Log §4
describes, so a later shred is a **tombstone the next export run carries**, not a
deletion anywhere downstream — a consequence of the geometry rather than a promise
by the exporter. The same question exists for **exported blobs** (Tenant &
Identity §2b, Artifact Store §6), where a copy is not column-shaped, and this
