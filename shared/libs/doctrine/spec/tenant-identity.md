...
| **Operator support access is membership, not an infra back door** | When operations staff must read a tenant's data to debug it, they do it as a **principal inside that tenant**: a support Role filled through a **gated, time-boxed grant**, whose every read is a **read event in the tenant's own log**, tenant-visible by default. There is no infra-level path that reads tenant data without appearing in the tenant's log. The grant expires; revoking it is an event | This is the same shape as §4's rule that staff are ordinary principals with a Task, a Gate and a trace — extended from _control-plane actions_ to a _support-read of tenant data_, which §4 left unhomed. Like Invariant 4, the claim must be public because the code enforcing it lives where outsiders cannot audit it; the enforcing **test lives in `cloud/`** while this claim lives here |
