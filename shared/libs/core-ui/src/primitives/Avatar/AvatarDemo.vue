...
// paints instead of falling back.
const workingSrc =
...
<template>
  <div class="flex flex-col gap-6">
    <div class="flex flex-col gap-2">
      <span class="text-xs text-muted-foreground">Ảnh hợp lệ</span>
      <Avatar :src="workingSrc" alt="Ada Lovelace" fallback="AL" />
    </div>

    <div class="flex flex-col gap-2">
      <span class="text-xs text-muted-foreground">Ảnh lỗi (fallback về chữ đầu tên)</span>
      <Avatar src="https://broken.invalid/not-found.jpg" alt="Grace Hopper" fallback="GH" />
    </div>

    <div class="flex flex-col gap-2">
      <span class="text-xs text-muted-foreground">Không có ảnh</span>
