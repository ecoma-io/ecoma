...
describe("InlineError", () => {
  ...
  it("forwards fallthrough attrs (e.g. data-testid) to the alert root — no inheritAttrs override", () => {
    const wrapper = mount(InlineError, {
      props: { message: "err" },
      attrs: { "data-testid": "dashboard-open-error" },
    });
    expect(wrapper.get('[role="alert"]').attributes("data-testid")).toBe("dashboard-open-error");
  });
});
