 ...
 * sit *inside* the field and share its focus bloom, instead of each caller
 * re-inventing an icon-inside-input layout. For numeric entry reach for
 * NumberField; for a bounded continuous value, Slider.
 *
 ...
 * of the attrs (`id`, `name`, `data-testid`, autocomplete) belong on the real
 * `<input>` — the split-attrs convention shared with Select. The accessible
 * name is a first-class prop pair (`aria-label`/`aria-labelledby`, camelized by
 * Vue) rather than a loose fallthrough attr, so it is bound onto the input
 * explicitly and shows up in the props table — a bare input primitive can only
 * be named by its host.
 */
