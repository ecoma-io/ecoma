...
import Badge, { type BadgeVariants } from "./Badge.vue";
...
describe("Badge", () => {
  it("paints each status variant in its own semantic token so a warning chip never reads as a success one", () => {
    const tokens: ReadonlyArray<[BadgeVariant, string]> = [
      ["neutral", "bg-subtle"],
  ...
  it("renders its slot content inline in a span so a chip can sit inside a sentence or a table cell", () => {
    const wrapper = mount(Badge, { slots: { default: "Đang chạy" } });
    expect(wrapper.element.tagName).toBe("SPAN");
    expect(wrapper.text()).toBe("Đang chạy");
