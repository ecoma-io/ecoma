...
// rendering (that belongs to `Toast`/`ToastItem`). Mocking `ToastItem` would
// remove the exact thing under test — the shared-viewport composition — and
// leave this file pinning nothing (`shared/libs/core-ui/CLAUDE.md`).

const items: ToastStackItem[] = [
  { id: 1, title: "Đã thêm thành viên", description: "John", variant: "success" },
  { id: 2, title: "Đã cập nhật quy trình", variant: "info" },
];
...
describe("ToastStack", () => {
  it("renders every simultaneous queue entry into ONE shared viewport so toasts stack instead of overlapping", async () => {
    ...
    expect(viewport.textContent).toContain("Đã thêm thành viên");
    expect(viewport.textContent).toContain("Đã cập nhật quy trình");
  });
  ...
  it("funnels a manual close through dismiss with the entry's id — the host queue stays the single source of truth", async () => {
    await mountStack(items);
    const closeButtons = document.querySelectorAll<HTMLButtonElement>("[aria-label='Đóng']");
