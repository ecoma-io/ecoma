 ...
 * full-bleed on an ultra-wide monitor (Design System › Principles §4), with
 * zero Tailwind breakpoints authored here or at the call site. The `minmax`
 * lower bound is wrapped in `min(100%, …)` so a tile never forces horizontal
 * overflow on a container narrower than `minTileWidth` itself.
 *
...
// tiles buys no separation the tile borders do not already give, it just pushes
// the second tile off screen. From `sm` up each step is its documented value.
const gapClass = {
  sm: "gap-2 sm:gap-3",
  md: "gap-3 sm:gap-4",
  lg: "gap-4 sm:gap-6",
