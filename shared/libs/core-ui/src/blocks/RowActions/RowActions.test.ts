 ...
 *
 * RowActions has no script — its entire contract is which utilities are on the
 * wrapper, so those class names ARE the mechanism, not an implementation
 * detail standing in for one. jsdom does not evaluate media queries or
 * `:hover`, so a test here cannot watch the reveal happen; that was exercised
 * against a real Chromium with touch emulation (opacity 0 on a fine pointer,
 * 1 on a coarse one). What these tests hold is the shape the browser then
 * acts on — and every one of them guards a failure that is invisible on the
 * pointer path, which is exactly the class of bug this component exists to
 * prevent.
 */
const ACTIONS = {
  default: `
    <button aria-label="Xem chi tiết">eye</button>
    <button aria-label="Xóa">trash</button>
  `,
};
...
describe("RowActions", () => {
  it("keeps every action mounted and named while at rest, so a keyboard or screen-reader user can still reach one", () => {
    ...
    expect(buttons.map((b) => b.attributes("aria-label"))).toEqual(["Xem chi tiết", "Xóa"]);
  });
  ...
  it("drops the reveal outright for a coarse pointer, which has no hover state to enter in the first place", () => {
    ...
    // already had to land on something invisible. So the refinement is skipped
    // rather than left as the only way in.
