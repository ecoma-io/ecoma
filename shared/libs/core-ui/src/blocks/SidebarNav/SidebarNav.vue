...
/**
 * SidebarNav — the operator console's primary NAVIGATION chrome: a `<nav>`
 * landmark of sections and destinations, pinned to the sunken plane (Design
 * System › Elevation — "sidebar/rail is the chrome layer, never lighter than
 ...
 * (Tooltip's own contract — see its doc header — is `aria-describedby`,
 * never the *only* source of a name) and its Tooltip doubles as the visible
 * hint. Native `<a>`/`<button>` elements are used throughout, so Enter/Space
 * activation and tab order come from the platform, not custom key handling.
 *
...
withDefaults(
  ...
  { collapsed: false, ariaLabel: undefined },
);
  ...
  return cn(
    ...
    item.active
      ...
      : // Hover lifts the row to the RAISED plane rather than tinting it with
        ...
        // (sunken < background < card, Design System › Elevation): the row the
        // pointer is on comes forward.
