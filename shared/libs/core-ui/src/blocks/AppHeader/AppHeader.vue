 ...
 * notifications + account cluster do not fit across a 390px viewport: measured
 * on the demo's own content, the field is left about 60px and renders as a
 * magnifier glyph pressed against the org name — a control that is technically
 * present and practically unusable. So the search takes a full-width second
 * row (`basis-full` + `order-last`), the header height goes from fixed to
 * `min-h-14`, and from `sm` up it snaps back to the single `flex-nowrap` row.
 * Wrapping rather than hiding is deliberate: the host slotted a search because
 * its users need one, and a block does not get to decide they need it less on
 * a phone.
 *
 * The `order-last` is the one thing to keep in mind if this changes: it moves
 * the search visually below the cluster while leaving it before the cluster in
 * the DOM, so on a phone the tab order reaches search first. That is the
 * better of the two mismatches — search is the more-used control — but it is a
 * mismatch, so do not extend `order-*` to the other regions.
 *
 * **Pinned, and separated by a hairline — never a shadow** (Design System ›
 * Elevation, "hairline over shadow" and "pinned chrome"). It sits on `bg-card`
 * (the raised plane), one step above the content it sits over, unlike
 * `PageHeader` which shares the content's own `bg-background` plane. Because
 * content scrolls *under* it, the surface is translucent + blurred so the
 * motion beneath stays legible as depth — the opaque `bg-card` is declared
 * first and stays the fallback wherever `backdrop-filter` is unsupported, so
 * the bar is never a see-through smear.
 *
 * Widths step rather than hold one value: gutters open up from `sm` and again
 * on wide desktops (`3xl`, 1920px), where a 16px gutter beside a 1920px canvas
 * reads as an accident (Principles §4 — expand with discipline, not just
 * shrink for mobile).
 *
 ...
 * something on it (Design System › Signature).
 */
