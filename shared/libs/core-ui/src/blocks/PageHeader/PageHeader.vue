 ...
 * surface: it pins to the top of the scroll container, carries the surface's
 ...
 *   `bg-background`, the same plane as the work area rather than the `sunken`
 *   navigation plane, because this is the surface's own header, not chrome
 *   that recedes. Rows scroll *under* it, so the plane is translucent +
 *   blurred; the opaque `bg-background` is declared first and remains the
 *   fallback wherever `backdrop-filter` is unsupported.
 * - **The count is a number, so it is `tabular`** (Principles §5) — a count
 *   that changes as items resolve must not shift the title beside it. It is
 *   deliberately NOT a Badge: a badge is a status chip, and "how many rows are
 *   below" is a fact, not a status. Wrap it in a Badge at the call site on the
 *   rare surface where the count itself is the alarm.
 *
 ...
 * nearest scrolling ancestor — a header that will not stick is almost always
 * an ancestor with `overflow: hidden`, not this component).
 *
 * Two width behaviors, both from Principles §4. Gutters step up (`sm`, then
 * `3xl`) rather than holding one value, so the band is not padded like a phone
 * on a 1920px canvas. And the description is capped at `max-w-prose` instead
 * of tracking the viewport — a one-line orientation stretched across an
...
withDefaults(
  ...
  { description: undefined, count: undefined },
);
</script>
