...
const ACTIONS = { actions: '<button type="button">Tạo quy trình</button>' };

describe("PageHeader", () => {
  it("renders the count as tabular so a title beside a number that keeps changing never shifts", () => {
    const wrapper = mount(PageHeader, { props: { title: "Quy trình", count: 12 } });
    const count = wrapper.get(".tabular");

    expect(count.text()).toBe("12");
    // `.tabular` is mono + tabular-nums (Principles §5). A proportional 8 is
    // wider than a 1, so a count ticking down as rows resolve would nudge the
    // heading left and right on every update.
  ...
  it("omits the count entirely when the host passes none, rather than falling back to a zero that reads as a broken fetch", () => {
    const wrapper = mount(PageHeader, { props: { title: "Quy trình" } });
  ...
  it("right-aligns the actions from the cluster itself, so they stay right after wrapping to their own row", () => {
    const wrapper = mount(PageHeader, { props: { title: "Quy trình" }, slots: ACTIONS });
    const cluster = wrapper.get("button").element.parentElement as HTMLElement;
  ...
  it("caps the description at a readable measure instead of tracking the viewport out to ultrawide", () => {
    const wrapper = mount(PageHeader, {
      props: { title: "Quy trình", description: "Quy trình đã dựng — chạy, theo dõi, và duyệt." },
    });
