...
// The first `go test` on a cold runner compiles the stdlib for coverage from
// scratch (measured 16s+ in CI, where GOCACHE starts empty), so every case
// gets a timeout sized to that, not to vitest's 5s default.
const GO_TIMEOUT = 120_000;
...
describe("runGoTests against real go modules", () => {
  ...
  it(
    "fails a module whose coverage sits below the floor",
    () => {
      process.chdir(
        goModule({
          "go.mod": GO_MOD,
  ...
  it(
    "propagates a failing test as a red run",
    () => {
      process.chdir(
        goModule({
          "go.mod": GO_MOD,
  ...
  it(
    "passes a skeleton with test files but no statements — the workspace's ◆G0 state",
    () => {
      process.chdir(
        goModule({
          "go.mod": GO_MOD,
