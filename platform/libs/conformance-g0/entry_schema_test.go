...
//
// TODO: required fields — id, the stream the entry belongs to and its position
// in that stream, timestamp, kind, the entry's schema version, the full actor
// identity, entity@version references, run_kind with its test-run id, and the
// payload (inline, or a hash into the Artifact Store).
